subroutine b4step1(mbc,mx,meqn,q,xlower,dx,t,dt,maux,aux)
    ...
    real(kind=8) :: posfr(1-mbc:mx+mbc), depth(1-mbc:mx+mbc)
    
    ! open file where front position is recorded
    if (abs(t-nint(t)) .le. 1.e-2) then
      open(unit=out_unit, file='front.txt', form='formatted',action='write',position='append'  )
      do i=1,mx
        posfr(i) = xlower + (i-0.5d0)*dx
        depth(i) = q(1,i)
      enddo
      where (depth .le. 0.001) posfr = 0.d0
      write(out_unit, "(50e26.16)") t,  maxval(posfr), abs(t-nint(t))
    close(out_unit)
    endif
